...
├── assets/img/headshot.jpg     Portrait (headshot-sm.jpg is the small version)
├── assets/img/alexaLogo.*      Animated logo (gif + mp4 + webm)
├── assets/css/style.css        All styling (edit the palette here)
├── assets/css/carousel.css     Styling for the video carousel on /content/
...
├── assets/js/carousel.js       Drives the /content/ video carousel (don't edit)
├── assets/pokedex-addon/       Icon set for the Pokedex AddOn carousel banner
...
- **Add an event / talk:** add an entry to `assets/js/events.js` — see the
  "Events (self-updating)" section above. Nothing else to touch.
- **Add a video:** in `content/index.html`, copy an `<article class="media-item">`
  block inside `<div class="carousel-track">` and swap the YouTube ID in **two**
  places (the `<a href>` and the `<img src>`), plus the title and blurb. It joins
  the carousel automatically. Newest video goes first.
- **Add a podcast:** copy the `<article class="media-item">` block in the
  `.media-grid` under §02 Audio in `content/index.html`.
- The nav and footer are duplicated in each page (static site). If you change one,
  change it in all five pages (`index.html` plus `about/`, `events/`, `content/`,
  `contact/`) — or ask Claude to do it in one pass.
